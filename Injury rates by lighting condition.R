...
data$lighting_condition <- ifelse(data$lighting_condition %in% c("DAYLIGHT"), "Day",
                                 ifelse(data$lighting_condition %in% c("DARKNESS, LIGHTED ROAD"), "Night with street lights",
                                        ifelse(data$lighting_condition %in% c("DUSK"), "Dusk",
                                               ifelse(data$lighting_condition %in% c("DARKNESS"), "Night without street lights",
...
lighting_counts$Lighting <- factor(lighting_counts$Lighting, 
                                 levels = c("Day", "Night with street lights", "Night without street lights", "Dusk", "Dawn", "Other"))
...
lighting_counts$label <- ifelse(lighting_counts$Accident_Type == "Injury", 
                               ...
                               NA)
# Plot the data
ggplot(lighting_counts, aes(x = Lighting, y = Count, fill = Accident_Type)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = label), 
            position = position_dodge(width = 0.9), 
            vjust = -0.5, 
            size = 3, na.rm = TRUE) +
  scale_y_continuous(breaks = seq(0, max(lighting_counts$Count, na.rm = TRUE), by = 5000)) +  # More ticks
  labs(
    title = "Comparison of Injury vs Non-Injury Accidents by Lighting Condition",
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        ...
        plot.title = element_text(family = "Arial", face = "bold", size = 12, color = "black", hjust = 0.25))  # Center title
